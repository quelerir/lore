...
Ассистент отвечает на вопросы по документам компании: файлы разложены в
«TOAST-слой» Postgres (реестр `lore_core` + физические таблицы
`splitter_toast.toast_tbl_*`; в комплекте — синтетический демо-слепок,
БД `lore_data`). Два режима (переключатель в сайдбаре, действует на новый
чат; у существующего чата режим зафиксирован):

- **Быстрый** (`fast`, по умолчанию) — langgraph с фиксированным
  маршрутом: discovery таблиц → один SQL (LLM) → выполнение под
  guardrails → ответ. Предсказуем и подходит маленьким моделям.
- **Умный** (`deep`) — deepagents со свободной оркестрацией инструментов
  discovery/inspect/select. Для сложных вопросов и сравнений.

Guardrails по контракту (см.
`docs/superpowers/specs/2026-07-15-campaign-agents-design.md`): только
SELECT, allowlist схем, policy gate для персональных данных, честный
no-table-answer вместо выдумки. Прогон eval-кейсов:
