...
- **Быстрый** — фиксированный маршрут: поиск таблиц документа → один
  SQL → ответ. Хорош для типовых вопросов.
- **Умный** — deepagents сам планирует поиск, изучение таблиц и SQL.
  Для сравнений и сложных вопросов (медленнее).

Вопросы, на которые отвечают демо-данные:

- «Какая разница между миддлом и Group Head в отделе контекстной
  рекламы?» — JOIN трёх таблиц матрицы компетенций;
- «Какие ФИО у юристов агентства?» — реестр сотрудников (плюс
  header-подсказка о записи, потерянной при извлечении);
- «Когда отпуск у Орловой Марии?» — отказ policy gate (персональные
  данные);
- «Сколько следов дают за активности в клубах?» — честный
  no-table-answer (таких таблиц нет).

Прогон eval-набора по этим кейсам: `python3 infra/eval-agents.py`
