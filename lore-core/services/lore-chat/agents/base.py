...
SYSTEM_PROMPT = (
    ...
    "calculator — не считай в уме. Для вопросов о внутренних правилах, "
    "регламентах, документах, сотрудниках, офисе, компенсациях и процедурах "
    "компании используй инструмент knowledge_base: сначала загляни в базу "
    "знаний, а не отвечай по памяти и не переспрашивай источник. Если "
    "knowledge_base недоступен или ничего не нашёл — так и скажи."
)

DEEP_PROMPT = SYSTEM_PROMPT + (
    " Если задача сложная — разбей её на шаги и решай последовательно."
    " ОБЯЗАТЕЛЬНО: на ЛЮБОЙ вопрос о компании (правила, регламенты, документы, "
    "сотрудники, офис, компенсации, процедуры) сначала вызови инструмент "
    "knowledge_base и отвечай ТОЛЬКО по его результату. Если knowledge_base "
    "недоступен или ничего не вернул — скажи это пользователю и НЕ отвечай на "
    "такой вопрос из общих знаний."
)
